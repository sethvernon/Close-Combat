...
void Position::from_fen(const string& fen) {
  ...
  for (Square sq = SQ_A1; sq <= SQ_H8; sq++)
      castleRightsMask[sq] = NO_CASTLES; // MSM  ALL_CASTLES;
...
//  assert(color_of_piece_on(from) == us);   // sav 2.25.14
...
void Position::do_capture_move(Key& key, PieceType capture, Color them, Square to, bool ep) {
    ...
    assert(capture != KING);  // SAV 2.25.14
...
//  assert(piece_on(rfrom) == piece_of_color_and_type(us, ROOK)); //sav 2.25.14
...
//  assert(square_is_empty(from)); SAV 2.25.14
//  assert(color_of_piece_on(to) == us);  SAV 2.25.14
  assert(!pm || relative_rank(us, to) == RANK_8);
...
void Position::allow_oo(Color c) {
  ...
  // st->castleRights |= (1 + int(c));
    st->castleRights |= NO_CASTLES; //MSM
}
