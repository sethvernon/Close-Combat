...
@property (nonatomic, strong)Options *qwOptions;

@end

@implementation BoardViewController

@synthesize boardView, gameController, blackClockView, whiteClockView, analysisView, bookMovesView, searchStatsView, moveListView;

/// *** Chess Model *** /// //// Lazy Instantiation // SV 11-26-2013 
...
//      [contentView addSubview: blackClockView];
      ...
      [self setView: rootView];
            ...
            // Board
      boardView =
         [[BoardView alloc] initWithFrame: CGRectMake(0.0f, 18.0f, 320.0f, 320.0f) GameName:self.gameName];
      [contentView addSubview: boardView];  
       NSLog(@"Loaded boardview");
...
//      [contentView addSubview: blackClockView];
      ...
      [contentView bringSubviewToFront: boardView];
       NSLog(@"Added subview boardview to contentView");
...
- (void)toolbarButtonPressed:(id)sender {
   ...
   // Ignore duplicate presses on the "Game" and "Move" buttons:
    if ([gameMenu isVisible] && [title isEqualToString: @"Game"])
      return;
    if ([moveMenu isVisible] && [title isEqualToString: @"Move"])
        return;
...
- (void)levelWasChanged {
    ...
    [gameController setGameLevel:[self.qwOptions gameLevel]];   ////  SAV 2.6.14
...
- (void)emailMenuCancelPressed {
   ...
   //   [emailMenu ];
    ...
    navigationController = nil; // ARc repair Seth 1/2/2014
}
...
//    [gameController connectToServer]; // removed atempt to connect to server //* Seth 12.30.13
}
