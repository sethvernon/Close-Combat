...
- (id)initWithBoardView:(BoardView *)bv        //// Testing for Storyboard init. /// Seth 1/4/14
   ...
   if (self == [super init]) {
      ...
      id sndpath = [[NSBundle mainBundle] pathForResource: @"Click"
                                                   ofType: @"wav"
                                              inDirectory: @"/"];
      CFURLRef baseURL = (CFURLRef)CFBridgingRetain([[NSURL alloc] initFileURLWithPath: sndpath]); /// Automatic ARC repair by Xcode..//* seth 12.30.13
...
- (void)startNewGame {
   ...
   [boardView stopHighlighting];
                        game = nil;  /// ** removed [game release] from stockfish ver /// seth 12.30.13

   for (PieceImageView *piv in pieceViews)
      [piv removeFromSuperview];
                        pieceViews = nil; /// ** removed [pieceViews release] *** for ARC added nil. /// seth 12.30.13
...
- (void)putPiece:(Piece)p on:(Square)sq {
   ...
   rect.origin = CGPointMake((int(sq)%8) * sqSize, (7-int(sq)/8) * sqSize);  //// changed from 8 to 10 SAV 2.10.14
...
- (void)showPiecesAnimate:(BOOL)animate {
   float sqSize = [boardView sqSize];
   CGRect rect = CGRectMake(0.0f, 0.0f, sqSize, sqSize);
   for (Square sq = SQ_A1; sq <= SQ_H8; sq++) {      /// SQ_A1 - SQ_H1
...
{    
    static NSString *pieceImageNames[16] = {
        nil, @"WPawn", @"WKnight", @"WBishop", @"WRook", @"WQueen", @"WKing", nil,   //// SAV Changing Pieces 2.17.14 Changed the Bishops to Queens.
        nil, @"BPawn", @"BKnight", @"BBishop", @"BRook", @"BQueen", @"BKing", nil
    };
    NSString *pieceSet = @"Alpha";  ////  [[Options sharedOptions] pieceSet]
...
- (void)gameFromPGNString:(NSString *)pgnString {
                            game = nil;  /// ARC repair // removed release // Seth 12.30.13

      for (PieceImageView *piv in pieceViews)
      [piv removeFromSuperview];
                            pieceViews = nil;   /// ARC repair // removed release // Seth 12.30.13
...
// Chage the "square" setting for the kings to a random location
-(void)randomKingLocations
{
    
//    NSArray *kingSquaresBlack = @[@"A1", @"B1", @"C1", @"D1", @"E1", @"F1", @"G1", @"H1"];
//    NSArray *kingSquaresWhite = @[@"A8", @"B8", @"C8", @"D8", @"E8", @"F8", @"G8", @"H8"];   /// ? need this?
...
//    enum Square {SQ_A1, SQ_B1, SQ_C1, SQ_D1, SQ_E1, SQ_F1, SQ_G1, SQ_H1};
    
//    NSMutableArray *squares_White = [NSMutableArray  arrayWithArray:@[@(SQ_A1), @(SQ_B1), @(SQ_C1), @(SQ_D1), @(SQ_E1), @(SQ_F1), @(SQ_G1), @(SQ_H1)]];
//    NSMutableArray *squares_Black = [NSMutableArray arrayWithArray:@[@(SQ_A8), @(SQ_B8), @(SQ_C8), @(SQ_D8), @(SQ_E8), @(SQ_F8), @(SQ_G8), @(SQ_H1)]];
    ...
    [self gameFromFEN: ccFEN];
//    for (Square sq = SQ_A1; sq <= SQ_H1 ; sq++) {
//        
//    }
    
    
//    for [Square *square in squares_White]
...
- (void)dealloc {
    ...
    remoteEngineController = nil;
    [engineController quit];
    game = nil;
    pieceViews = nil; // Should we remove them from superview first??
//    for (Piece p = WP; p <= BK; p++)
//        pieceImages[p];
    engineController = nil;
